/*
  # Complete AskStan Platform Database Schema - Fixed
...
DROP VIEW IF EXISTS user_growth_summary CASCADE;
...
CREATE TABLE growth_metrics (
  ...
  -- Unique constraint for one metric per user/platform/type/date
...
CREATE INDEX billing_history_amount_idx ON billing_history(amount);
...
CREATE INDEX growth_metrics_user_id_idx ON growth_metrics(user_id);
CREATE INDEX growth_metrics_platform_idx ON growth_metrics(platform);
CREATE INDEX growth_metrics_recorded_at_idx ON growth_metrics(recorded_at);
CREATE INDEX growth_metrics_recorded_date_idx ON growth_metrics(recorded_date);
CREATE INDEX growth_metrics_composite_idx ON growth_metrics(user_id, platform, recorded_date);

CREATE INDEX content_templates_platform_idx ON content_templates(platform);
CREATE INDEX content_templates_category_idx ON content_templates(category);
CREATE INDEX content_templates_premium_idx ON content_templates(is_premium);
CREATE INDEX content_templates_usage_idx ON content_templates(usage_count);

CREATE INDEX user_achievements_user_id_idx ON user_achievements(user_id);
CREATE INDEX user_achievements_platform_idx ON user_achievements(platform);
...
-- Function to update subscription from Stripe webhook (FIXED PARAMETER ORDER)
...
-- Function to record billing events (FIXED PARAMETER ORDER)
...
-- Function to award achievements (FIXED PARAMETER ORDER)
...
GRANT EXECUTE ON FUNCTION record_billing_event(text, text, integer, text, text, text, text, text, timestamptz) TO service_role;
